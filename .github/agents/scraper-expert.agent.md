...
8. **When editing `main.py` for any reason**: run the SCRAPERS audit (Phase 3 step 4) immediately after — even chore/refactor commits can silently drop registrations.
    ...
    12. **google_news_rss redirect URLs**: `news.google.com/rss/articles/...` URLs cannot be resolved server-side (requests → 400, Playwright → bot detection). Leave as-is; they work in real browsers. Set `_STALE_DAYS = 21` — redirect URLs expire in ~2–3 weeks.
