...
10. **Cinema scrapers — `end_date` と `business_hours` 必須:** Cinema scraper には `end_date` と `business_hours` の正確な設定が義務付けられている。3タイプに分類: **Type 1**(票務平台分離型: `end_date`=木曜末日, `business_hours`=票務サイトから), **Type 2**(排片表嵌入型: `end_date`=max(dates), `business_hours`=ページ内schedule), **Type 3**(上映中リスト型: `end_date`=ラベルから, `business_hours`=詳細ページ or None). 共通禁止: `end_date=start_date`, 空文字列 `business_hours=""`, 推測 `end_date`. `raw_description` に上映期間全体を記載しないと annotator SINGLE-DAY RULE が `end_date` を上書きする。詳細: `SKILL.md § Cinema scraper — end_date と business_hours 完全規則`. 稽核表は同 SKILL.md セクション末尾参照。
