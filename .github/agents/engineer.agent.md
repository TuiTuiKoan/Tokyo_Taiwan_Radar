...
10. **Cinema scrapers — `end_date` と `business_hours` 必須:** Cinema scraper には `end_date` と `business_hours` の正確な設定が義務付けられている。3タイプに分類: **Type 1**(票務平台分離型: `end_date`=木曜末日, `business_hours`=票務サイトから), **Type 2**(排片表嵌入型: `end_date`=max(dates), `business_hours`=ページ内schedule), **Type 3**(上映中リスト型: `end_date`=ラベルから, `business_hours`=詳細ページ or None). 共通禁止: `end_date=start_date`, 空文字列 `business_hours=""`, 推測 `end_date`. **`event_form=["film_screening"]` 必須**（`"screening"` は無効値 → DB check constraint エラー）. `raw_description` に上映期間全体を記載しないと annotator SINGLE-DAY RULE が `end_date` を上書きする。**Type 3 で end_date 取得不可の場合は date prefix を入れない**（SINGLE-DAY RULE 誤発動防止）. JST ISO datetime は `.replace("+09:00","")` 禁止 → `datetime.fromisoformat(dt)` + `datetime(y,m,d, tzinfo=utc)`. 詳細: `SKILL.md § Cinema scraper — end_date と business_hours 完全規則`. 稽核表は同 SKILL.md セクション末尾参照。
...
38. **Admin mutation 成對原則（confirm / dismiss / toggle）：** Admin page の paired mutation（confirm/dismiss、active/inactive、approve/reject など）は**必ず同じ実装パターンを使う**。`confirmReport` が server action なら `dismissReport` も server action でなければならない。片方のみ昇格させると、残った方が 0-row silent success トラップに落ちる（2026-05-15 `handleDismiss` 事例）。新 admin mutation を追加する前に配対 handler を確認し、server action として揃える。See `SKILL.md § Admin Mutation 成對原則`.
...
42. **Client Component 直接 INSERT 禁止（ユーザー向けフォーム含む）：** `supabase.from(...).insert()` を Client Component から直接呼ぶのは禁止。ブラウザ→PostgREST fetch がハングすると `try/catch` は発動せず `setStatus("error")` も呼ばれない → 永久 loading 状態（2026-05-15 `ReportSection` commit `53445be`）。anon INSERT を RLS で許可済みでもブラウザ直接 INSERT はハング耐性がない。必ず Server Action を経由する。See `SKILL.md § Client Component 直接 INSERT 禁止`.
