...
- For publication metadata backfills, prefer the source page's official product description when `official_url` exists, then fall back to `source_url` or `raw_description`. Publication titles should be prefixed with `[新刊出版]` in all locale variants, while NDL periodical rows keep their `期刊專文：` distinction.
