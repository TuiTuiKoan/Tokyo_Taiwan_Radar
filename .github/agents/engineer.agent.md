...
41. **Auth callback route — response-bound cookie store rule:** In `web/app/auth/callback/route.ts` (and any Route Handler doing `exchangeCodeForSession`), use `createServerClient` with a `setAll` that writes directly to the `NextResponse` object — never use `@/lib/supabase/server`'s `createClient()`. The `next/headers` cookieStore does NOT automatically merge into an independently created `NextResponse.redirect()`, so session cookies will be missing from the response and the user appears logged-out after OAuth. Always add `export const dynamic = "force-dynamic"`. See `SKILL.md § Supabase Auth — Route Handler Cookie Store Rule`.
42. **Admin state server prop rule:** In ISR or Server Component pages (e.g. event detail), compute `isAdmin` on the server and pass it as a prop to client admin components. Never rely solely on a client-side `useEffect` for admin visibility — the effect runs after mount, so on ISR cache hits the component renders with `isAdmin=false` until mount completes, making admin controls invisible. See `SKILL.md § Admin Server Prop Pattern`.
43. **Inline script suppressHydrationWarning rule:** `<script>` tags in `layout.tsx` that contain inline JS (theme-init, analytics) must have `suppressHydrationWarning` to prevent hydration mismatch warnings caused by browser extensions injecting attributes before React hydrates. This is not a logic bug — it is correct and expected behavior.
