...
24. **Movie event person name enrichment:** When movie events have GPT-generated descriptions, always run `python annotator.py --enrich-person-names` after annotation to fix wrong phonetic name translations. GPT translates katakana names phonetically (e.g. ギデンズ・コー → 「紀德恩」instead of 「九把刀」). The person name lookup chain uses eiga.com → zh/ja Wikipedia. See `SKILL.md § Person Name Lookup Pattern`.
