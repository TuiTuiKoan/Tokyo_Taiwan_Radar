...
9. **Cinema scrapers — Movie Title Lookup:** When implementing or updating a cinema scraper (`category=["movie"]`), always call `lookup_movie_titles(name_ja)` from `scraper/movie_title_lookup.py` before constructing `Event()`. For Taiwan-produced films not found on eiga.com, check **GHFF** (金馬獎 `goldenhorse.org.tw/film/about/archive/`) as the authoritative source for official Chinese/English titles and director name. Priority: **GHFF > eiga.com > GPT**. Key pitfalls: (a) `performers_zh[]` must contain real Chinese names — NOT katakana mechanical transliterations (`艾絲特·劉` ← エスター・リウ is wrong; the real name is `劉品言`); (b) Kanji-1-char-off errors in director names (`練` vs `連`) are visually hard to spot — always copy-paste from the authority source; (c) When fixing `name_zh`/`name_en`, also update `description_zh/en` (replace old title) AND `works` table fields simultaneously. See `SKILL.md § 台湾映画の権威ソース優先順位` + `§ performers_zh[] 機械音訳禁止ガード`.
