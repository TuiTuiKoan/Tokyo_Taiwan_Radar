...
## 2026-06-02 — `講師` / `主催` ラベル未抽出で organizer・performer が null（event `19aecffd`）

**問題：**
- 講演会ページに `講師:` と `主催:` が明記されているにもかかわらず、DB 上で `organizer` / `performer` が `null` のままになり、前端の主辦資訊區塊が非表示になった。

**根本原因：**
- scraper は講者ラベルを `講演者` のみ抽出しており、実ページで使われた `講師` を拾えなかった。
- `主催` を抽出せず、`raw_description` にも主催・講師の構造化行を書いていなかったため、annotator が補完できなかった。

**修正：**
- 講者抽出を fallback 化：`講演者` → `講師` → `登壇者` → `司会` → `報告者`。
- `主催` / `共催` 抽出を追加。
- `raw_description` に `主催:` / `共催:` / `講師:` を追記するよう変更。

**教訓：**
- Waseda の投稿テンプレートは記事ごとにラベル揺れがあるため、人物・主催系ラベルは複数パターンを同時に実装すること。
- annotator に依存する欄位は、本文自由文ではなく構造化行（`主催:`・`講師:`）として明示的に渡すこと。
