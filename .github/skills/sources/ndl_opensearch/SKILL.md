...
- **出版事件欄位模板**: `location_name = null`、`location_address` / `business_hours` / `price_info` は占位文字 `新書購買請洽各通路` を維持する。`performer` は作者、`organizer` は出版社扱い、`organizer_url` は出版社ホームページ、`official_url` は書籍詳細ページ、`event_form = ["publication"]`。
