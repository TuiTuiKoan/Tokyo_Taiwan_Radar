...
- **出版事件欄位模板**: 書籍系は `location_name = null`、`location_address` / `business_hours` / `price_info` は占位文字 `新書購買請洽各通路` を維持し、`performer` は作者、`organizer` は出版社名、`organizer_url` は出版社ホームページ、`official_url` は書籍公式詳細ページ、`event_form = ["publication"]` を基本とする。著者イベントは内容に応じて別途 lecture 分流する。
