...
**根本原因：**
1. 日本地址判定 regex 原本使用字首錨點 (`^`)，導致以 `〒` (郵遞區號) 開頭的地址無法匹配成功。
2. `annotator.py` 與 `cityLabel.ts` 缺乏台灣 22 縣市的提取邏輯（含 `臺/台` 歸一化）。
3. 混合型音樂/表演活動（現場+線上，如 `380c0ab2-1713-4bc9-86c5-6101d8ec741a`）缺乏統一展示規範，常被誤標為純線上。

**修復：**
1. 將 `extract_prefecture` (Python) 與 `extractCity` (TS) 的 regex 移除強字首錨點，改用搜尋模式。
2. 擴充支援台灣 22 縣市，並在提取前進行 `臺` 到 `台` 的歸一化。
3. 確立「現場+線上」混合場地標記規範：`location_name` 並列兩者（如 `會場 / オンライン`），且必須保留實體地址以利地圖與區域分群。對於 `performing_arts` 類型的演唱會/直播，必須確保兩個資訊都標齊。

**教訓：** 
- **Regex 錨點風險**：在處理 web 抓取地址時，不可假設地址一定從「都道府縣」開始，郵遞區號或雜訊常會領先出現。
- **混合場地鎖定（兩個都標）**：對於 Hybrid 活動，務必手動鎖定 `field_corrections`，防止 AI 看到「線上」關鍵字就將實體地址與縣市抹除。記得現場名稱與線上都要標。
