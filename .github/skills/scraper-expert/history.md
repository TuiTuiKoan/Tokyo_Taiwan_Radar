...
**After (corrected):** tv_program, geopolitics, academic
**Admin notes:** —
**Action:** Category corrected inline — event remains active (is_active=true, annotation_status=annotated).
---
...
**Error**: `gguide_tv` scraped 「Let's try ソルトゲーム 14 仙台湾SLJ」 as a Taiwan-related event. `仙台湾` = 仙台 (Sendai, city) + 湾 (bay) — completely unrelated to 台湾.
The `_is_taiwan_title()` function matched `"台湾"` as a substring of `"仙台湾"`.

**Fix**: Added `_FALSE_POSITIVE_PATTERNS = re.compile(r"仙台湾")` to `gguide_tv.py`. `_is_taiwan_title()` now strips all false-positive patterns from the title before checking Taiwan keywords. If the cleaned title still contains a Taiwan keyword, it is accepted; otherwise rejected.
Also deactivated the erroneous DB record (`id=421d763d`).

**Lesson**: When `台湾` is used as a keyword for simple substring search, it will match:
- `仙台湾` — Sendai Bay (宮城県)
- Any future Japanese place name ending in `台湾X` or `X台湾`
Guard: maintain `_FALSE_POSITIVE_PATTERNS` regex in each scraper that does substring matching. Strip matched patterns first; if no Taiwan keyword remains, reject. → Added to SKILL.md `## gguide_tv-specific`.
