...
## 阪急阪神 H2O CMS — hankyu (umeda/hakata/kobe) + hanshin (umeda)
...
- **Seasonal pattern**: Taiwan events (e.g. 梅田 台湾ライフ autumn; 阪神の台湾展) are intermittent — a store returning 0 events off-season is **correct**, not a bug. All are weekly sources (`hankyu_umeda`/`hankyu_hakata`/`hankyu_kobe`/`hanshin_umeda`): present in `main.py WEEKLY_SOURCES`, `health_check.py NON_DAILY_SOURCES` + `ZERO_EVENT_OK_SOURCES`, and `qa_triage.py NON_DAILY_SOURCES`.
...
- **sources registry**: migration 091 registers `hankyu_hakata`/`hankyu_kobe` as `department_store`/`weekly` and sets `hankyu_umeda` frequency=weekly; migration 092 registers `hanshin_umeda` (`department_store`/`weekly`).
