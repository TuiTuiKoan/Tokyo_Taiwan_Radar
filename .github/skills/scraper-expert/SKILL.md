...
- **Taiwan roots category signal**: When a featured author, artist, creator, or performer is explicitly described as born in or from Taiwan (`台湾出身`, `台湾生まれ`, `Taiwan-born`, `born in Taiwan`, or a Taiwan city + `出身`), include `history` for the roots dimension. Do not add `history` for Taiwan university attendance, study-abroad history, or work activity in Taiwan alone.
- **Small venue exhibition organizer fallback**: For small gallery/shop/cafe/bookstore/craft-venue/museum exhibition pages hosted by the venue itself, the public-facing venue or shop name may be used as `organizer` when no `主催` label is present and the name appears in source text. Do not infer organizer from generic rental halls, universities, convention centers, aggregator platforms, or news/source names.
...
- **`Japan/Taiwan address extraction rule (2026-06-07)`**:
...
2. The annotator calls `venue_registry.lookup_venue_for_location()` to fill eligible `location_address`, `location_prefectures`, `venue_id`, `location_url`, and `business_hours` values.
...
6. **Hours precedence:** Keep current annotation schedules and stored event schedules ahead of authoritative venue general hours. Apply general hours only as a fill when both are empty. Every `field_corrections` key is locked, including an empty-string sentinel.
7. **First-party source gate:** Take general venue hours only from a venue or operator's first-party `アクセス`, store, visitor-information, or `営業時間` page. Store the official page in `homepage` and stable general hours in `venues.business_hours`. Never infer hours or promote a restaurant, cinema, or tenant exception to the whole venue.
8. **Subspace labels:** A `<canonical venue> + subspace suffix` label such as `expo`, `書籍レジ`, or `内 イベントスペース` may inherit the parent address, prefectures, venue ID, homepage, and eligible general hours. Preserve the more specific `location_name`, `location_name_zh`, and `location_name_en`. Canonical-prefix fallback requires an explicit suffix boundary and never uses a broad alias as a prefix.
...
### Encoding guard before classifying zero-event output
...
### Keyword-filtered long-tail sources
...
### Do not hide a fresh bug with ZERO_EVENT_OK
...
- **Retail and mall `business_hours`:** `venues.business_hours` is available after migration 081. Store reusable general hours in the authoritative venue seed and follow the first-party source gate and fill-only precedence above instead of relying only on per-event corrections. Event-specific schedules always win, and restaurant, cinema, or tenant exceptions never become whole-venue values.
  - Never guess without first-party confirmation. Incident `a4442567`: the guessed Lumine Est hours were `11:00〜22:30`, while the official shopping hours were weekdays `11:00〜21:00` and weekends or public holidays `10:30〜21:00`; restaurant hours were `11:00〜22:00`.
...
## 阪急阪神 H2O CMS — hankyu (umeda/hakata/kobe) + hanshin (umeda/nishinomiya/mikage/amagasaki)
...
- **Hanshin sister store (H2O group) reuses this**: `scraper/sources/hanshin.py` subclasses `_HankyuBase` and only overrides `_store` to resolve a Hanshin `_Store` registry — same H2O CMS, so all parsing (date parser, two-tier filter, `_fetch_taiwan_detail_evidence`, `_build_source_id`) is reused with zero changes to `hankyu.py`. Enabled stores (4): `HanshinUmedaScraper` → `hanshin_umeda` (阪神梅田本店/大阪, `/hshonten/event/`, hosts 「阪神の台湾展」) plus three 兵庫県 branches `HanshinNishinomiyaScraper` → `hanshin_nishinomiya` (阪神・にしのみや/西宮, `/nishinomiya/event/`), `HanshinMikageScraper` → `hanshin_mikage` (阪神・御影/神戸東灘, `/mikage/event/`), `HanshinAmagasakiScraper` → `hanshin_amagasaki` (あまがさき阪神/尼崎, `/amagasaki/event/`) — all on `www.hanshin-dept.jp`. Adding a Hanshin branch = one `_HANSHIN_STORES` config row + one concrete class. `_Store.base_url` already handles the different domain (`hanshin-dept.jp`). Don't rename `_HankyuBase`/`_Store` without updating `hanshin.py`.
