...
### E — 全国ブランドイベントの start_date = 最初の企画日（周年記念日ではない）

**問題：** `2cb72ee9`（鼎泰豐30周年）の `start_date = 2026-10-04`（30周年記念日）のまま確認していたが、実際の最初の企画は `2026-05-15`（小籠包11種セット販売開始）。記念日前に何も表示されない状態になっていた。
**修正：** `start_date = 2026-05-15`、`end_date = 2026-10-04`（周年記念日を最終日として保持）+ FC ロック。
**教訓：** 周年記念型イベントの `start_date` は「記念日」ではなく「最初の企画・告知が始まる日」にする。記念日は `end_date` に設定する（または name_ja に明記）。子活動の最小 `start_date` が親 `start_date` の正解として使える。

### F — ブランドイベントの location_name と location_address の分離

**問題：** `2cb72ee9` の `location_name = '鼎泰豐（東京・横浜・大阪・名古屋・福岡 他全国30店舗）'` と設定したが、UI 上の「会場」フィールドが冗長になった。ユーザーからの指摘で分離が必要と判明。
**修正：** `location_name = '鼎泰豐'`（ブランド名のみ）、`location_address = '東京・横浜・大阪・名古屋・福岡 他全国30店舗'`（都市列挙をプレーンテキストとして配置）。
**根拠：** `location_address` に実際の住所でないテキストを入れると、地図リンクが生成されずプレーンテキストとして安全に表示される。
**教訓：** 全国ブランドイベント：`location_name = ブランド名のみ`、`location_address = 都市列挙テキスト`。都市列挙を `location_name` に入れると UI 表示が崩れる。
