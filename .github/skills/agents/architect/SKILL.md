...
- **Rendered visual-delta acceptance guard（2026-08-11 教訓）**：任何要求顏色、明暗、透明度、字重或其他視覺差異的 `web/` 計畫，必須先量測變更前的 browser rendered baseline，並把 semantic token 追到最終 computed value。計畫需盤點 controlling global selector、component utility、inline style 與 specificity override；只驗 class、CSS source 或 token 名稱不構成視覺驗收。Acceptance 必須在真實目標元素上讀取 computed style（pseudo-element 也要直接讀，例如 `::placeholder`），並覆蓋 light／dark theme；無法由 computed style 表達的效果改用 screenshot pixel／visual assertion。另需做負向 mutation：還原舊 rendered value 後，回歸測試必須失敗。若新舊最終值相同，實作是 visual no-op，不得宣稱完成。
...
- **Spec ⟺ Worktree coupling（大型 feature 隔離；決策見 agent Gate，命令見 canonical）**：凡值得建 `docs/specs/active/<slug>/` 的長期 feature（跨 session／多檔／新模塊），計畫必須 provision 一個 `ttr-<slug>-worktree`（branch `feat/<slug>`），1:1 綁定、**僅適用規則上線後新建/重啟的 spec**（既有 spec grandfather）。小改動不建 spec、不建**專用** worktree，但**仍須先問使用者要用哪個現有 worktree**——主工作樹不是選項（見下條 Worktree 確認閘門）。並行 session 的 `git stash`/clean 碰不到 linked worktree，避免未 commit WIP 被當「unrelated before deploy」掃走（2026-06-29 v8 投稿精靈 WIP 散落 5 stash 事件）。**worktree 命令細節（state matrix、`.git/info/exclude`、STOP 條件）一律 defer `.github/instructions/git.instructions.md § Isolated worktree`，SKILL 只管「何時決策」。** runSubagent 同步但 return 常截斷——驗收看 git status/diff 不看回傳。
- **Worktree 確認閘門（實作前必問，不得推定）**：任何實作工作開始前，必須先向使用者確認在哪個 worktree 進行，得到明確答覆才動工；變更再小也不跳過。主工作樹（`Tokyo Taiwan Radar`／`main`）**僅供治理與盤點**——規劃、稽核、文件、spec 維護、狀態對帳；不得用於功能實作。計畫中必須寫明實作落在哪個 worktree，不可留白讓 Engineer 自行判斷。**這條規則是為了修掉「小改動走主工作樹」這個既有漏洞**：小改動同樣會累積成未提交 WIP，同樣會被平行 session 的 stash 掃走（2026-08-08：一批跨四天、11 檔、+550/−134 的 agent 教訓文件被發現長期懸在主工作樹未提交）。閘門實行細節 defer `git.instructions.md § Worktree confirmation gate`。
- **主工作樹髒污取證（確認閘門的前置步驟，非替代）**：實作前除了問使用者用哪個 worktree，還要先跑 `git --no-pager status --short` 檢查主工作樹。若主工作樹帶著其他 session 的未提交變更，即使只是小改動，也一律導回該工作線的 owning worktree 落地。本 repo 經常同時跑三個以上 session，因此「主工作樹是乾淨的」不能預設，每次都要取證。（2026-08-06 取證：一個懸置 21 小時的 stale `index.lock`；另一 session 在同一分支上的 commit 差點被一併 push；rebase 前有 11 個他人 WIP 檔案需先備份。）
