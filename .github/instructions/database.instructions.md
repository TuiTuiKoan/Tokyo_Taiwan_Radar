...
- **Maintenance lock (migration 094, decision-16a)**: fail-closed `public.admin_reports_maintenance_active()` (`SECURITY DEFINER STABLE`, `search_path = public`; returns `true`/DENY unless the `admin_reports_cleanup_maintenance` row exists with the exact JSON boolean predicate `value->'active' = 'false'::jsonb`). Missing keys, JSON null, string `"false"`, other malformed values, and a missing row all deny. Per-command RESTRICTIVE policies `<t>_maint_block_{insert,update,delete}` on `events`, `event_reports`, `field_corrections`, `category_corrections`, `selection_reason_corrections`, `works` AND with the permissive policies to block anon/authenticated writes while the lock is active — SELECT untouched, service_role unaffected (bypasses RLS). Migration is AUTHORED-only until decision-16b bring-up.
