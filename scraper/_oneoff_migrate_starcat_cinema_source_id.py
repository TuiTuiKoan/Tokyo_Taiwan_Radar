"""
一次性遷移腳本：starcat_cinema source_id 從 numeric-id-based 改為 name_ja hash-based。

舊格式：starcat_{thumbnail_id}
新格式：starcat_{md5(normalized_title)[:12]}（make_film_source_id）

用法：
  python _oneoff_migrate_starcat_cinema_source_id.py          # dry-run（預設，不改 DB）
  python _oneoff_migrate_starcat_cinema_source_id.py --apply  # 實際執行
"""
import os
import sys
