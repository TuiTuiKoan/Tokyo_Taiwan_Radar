"""
一次性遷移腳本：human_trust_cinema source_id 從 numeric-id-based 改為 name_ja hash-based。

舊格式：human_yurakucho_{movie_id}
新格式：human_yurakucho_{md5(normalized_title)[:12]}（make_film_source_id）

用法：
  python _oneoff_migrate_human_trust_cinema_source_id.py          # dry-run（預設，不改 DB）
  python _oneoff_migrate_human_trust_cinema_source_id.py --apply  # 實際執行
"""
import os
import sys
...
# DB source_name 與 source_id slug 不同：scraper 類別 SOURCE_NAME="human_trust_cinema"，
# 但 source_id 前綴使用 "human_yurakucho"（與 scraper 一致）。
SOURCE_NAME = "human_trust_cinema"
SLUG = "human_yurakucho"
...
def main() -> None:
    ...
    rename_only: list[tuple[str, dict]] = []        # (new_sid, event)
    merge_groups: list[tuple[str, dict, list[dict]]] = []  # (new_sid, primary, secondaries)
