...
export default function Button({ loading = false, variant = "primary", children, className = "", disabled, type, ...props }: Props) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-60 ${variantClassName[variant]} ${className}`}
      {...props}
    >
      {loading && (
